...
- **Change root** — jump to any directory with `gd` without losing your sort and view settings. The picker's path field and shortcut list are separate panes; `Tab` switches between them, and the focused one is outlined in cyan. Save the places you use with `a` (and forget them with `d`); favourites are starred, ordered most-recently-visited first alongside the built-in locations, and stored in `~/.config/myd/hosts.toml`.
