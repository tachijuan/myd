...
- **Saved apps** — the dialog also keeps the programs you actually use, in the same shape as the `gd` picker: a list under the command field, and an actions panel reached with `Tab`. The arrows walk the list and `Enter` runs the highlighted program; typing jumps to an entry, and a letter matching nothing goes to the command field instead. `S`ave keeps what is in the field, `U`pdate replaces the highlighted entry's command, `F`orget removes it. With nothing saved yet, typing a command is enough for `S`ave to appear — that is how the first entry is made; with an empty field and an empty list the section is absent entirely. An entry can claim file types, and one that claims the current file sorts to the top with a `•` beside it — pressing `O` on a photograph offers the image editor first, without hiding anything else. Stored in `~/.config/myd/apps.toml`, hand-editable:
  ...
  `match` is optional; without it the entry is offered for any file. Patterns are extensions — `*.png`, `.png` and `png` all mean the same — matched case-insensitively, so a camera writing `.JPG` needs no entry of its own.
